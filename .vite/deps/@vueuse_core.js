// node_modules/.pnpm/@vueuse+shared@8.7.5/node_modules/@vueuse/shared/index.mjs
...
function reactiveOmit(obj, ...keys2) {
  return reactiveComputed(() => Object.fromEntries(Object.entries(toRefs$1(obj)).filter((e) => !keys2.includes(e[0]))));
}
...
// node_modules/.pnpm/@vueuse+core@8.7.5/node_modules/@vueuse/core/index.mjs
import { isRef as isRef2, ref as ref2, watchEffect as watchEffect2, computed as computed2, inject as inject2, unref as unref2, watch as watch2, getCurrentInstance as getCurrentInstance2, customRef as customRef2, onUpdated, reactive as reactive2, shallowRef as shallowRef2, onMounted as onMounted2, markRaw, getCurrentScope as getCurrentScope2, nextTick as nextTick2, isVue2 as isVue22, set as set2, del, readonly as readonly2, onBeforeUpdate } from "vue-demi";
...
function useClamp(value, min, max) {
  ...
  return computed2({
    get() {
      return _value.value = clamp(_value.value, unref2(min), unref2(max));
    },
    set(value2) {
      _value.value = clamp(value2, unref2(min), unref2(max));
    }
  });
}
function useClipboard(options = {}) {
  ...
  async function copy(value = unref2(source)) {
...
function useStorage(key, initialValue, storage, options = {}) {
  ...
  if (!storage)
    return data;
  const rawInit = unref2(initialValue);
...
function useColorMode(options = {}) {
  ...
  const store = storageRef || (storageKey == null ? ref2("auto") : useStorage(storageKey, "auto", storage, { window: window2, listenToStorageChanges }));
...
function useCssVar(prop, target, { window: window2 = defaultWindow, initialValue = "" } = {}) {
  ...
  watch2([elRef, () => unref2(prop)], ([el, prop2]) => {
    if (el && window2) {
  ...
  watch2(variable, (val) => {
    ...
    if ((_a2 = elRef.value) == null ? void 0 : _a2.style)
      elRef.value.style.setProperty(unref2(prop), val);
  });
  return variable;
}
...
function useDevicePixelRatio({
  ...
  const pixelRatio = ref2(window2.devicePixelRatio);
  ...
  DEVICE_PIXEL_RATIO_SCALES.forEach((dppx) => {
    ...
    watch2([mqlMin, mqlMax], handleDevicePixelRatio);
  });
  return { pixelRatio };
}
...
function useDraggable(target, options = {}) {
  ...
  const position = ref2((_b = options.initialValue) != null ? _b : { x: 0, y: 0 });
  ...
  const handleEvent = (e) => {
    if (unref2(options.preventDefault))
      e.preventDefault();
    if (unref2(options.stopPropagation))
      e.stopPropagation();
  };
  const start = (e) => {
    var _a22;
    if (!filterEvent(e))
      return;
    if (unref2(options.exact) && e.target !== unref2(target))
      return;
    const rect = unref2(target).getBoundingClientRect();
    const pos = {
      x: e.pageX - rect.left,
      y: e.pageY - rect.top
    };
  ...
  return __spreadProps$42(__spreadValues$c({}, toRefs(position)), {
    position,
...
function useElementByPoint(options) {
  ...
  const controls = useRafFn(() => {
    element.value = document.elementFromPoint(unref2(x), unref2(y));
  });
...
function useElementVisibility(element, { window: window2 = defaultWindow, scrollTarget } = {}) {
  ...
  const testBounding = () => {
    ...
    if (!unref2(element)) {
      ...
      const rect = unref2(element).getBoundingClientRect();
  ...
  if (window2)
    tryOnMounted(() => useEventListener(unref2(scrollTarget) || window2, "scroll", testBounding, { capture: false, passive: true }));
...
function useFavicon(newIcon = null, options = {}) {
  ...
  const favicon = isRef2(newIcon) ? newIcon : ref2(newIcon);
...
function createFetch(config = {}) {
  ...
  function useFactoryFetch(url, ...args) {
    const computedUrl = computed2(() => config.baseUrl ? joinPaths(unref2(config.baseUrl), unref2(url)) : unref2(url));
...
function useFetch(url, ...args) {
  ...
  const execute = async (throwOnFailed = false) => {
    ...
    if (config.payload) {
      ...
      defaultFetchOptions.body = config.payloadType === "json" ? JSON.stringify(unref2(config.payload)) : unref2(config.payload);
    }
    let isCanceled = false;
    const context = { url: unref2(url), options: __spreadValues$a(__spreadValues$a({}, defaultFetchOptions), fetchOptions), cancel: () => {
      isCanceled = true;
    } };
  ...
  watch2(() => [
    unref2(url),
    unref2(options.refetch)
  ], () => unref2(options.refetch) && execute(), { deep: true });
  const shell = {
    isFinished,
    statusCode,
    response,
    ...
    return (payload, payloadType) => {
      if (!isFetching.value) {
        ...
        if (isRef2(config.payload)) {
          watch2(() => [
            unref2(config.payload),
            unref2(options.refetch)
          ], () => unref2(options.refetch) && execute(), { deep: true });
        }
        if (!payloadType && unref2(payload) && Object.getPrototypeOf(unref2(payload)) === Object.prototype)
...
function useFileSystemAccess(options = {}) {
  ...
  async function open(_options = {}) {
    if (!isSupported)
      return;
    const [handle] = await window2.showOpenFilePicker(__spreadValues$9(__spreadValues$9({}, unref2(options)), _options));
  ...
  async function create(_options = {}) {
    if (!isSupported)
      return;
    fileHandle.value = await window2.showSaveFilePicker(__spreadValues$9(__spreadValues$9({}, unref2(options)), _options));
    data.value = void 0;
    await updateFile();
    await updateData();
  }
...
var useImage = (options, asyncStateOptions = {}) => {
  const state = useAsyncState(() => loadImage(unref2(options)), void 0, __spreadValues$82({
    resetOnExecute: true
  }, asyncStateOptions));
  watch2(() => unref2(options), () => state.execute(asyncStateOptions.delay), { deep: true });
  return state;
};
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  const state = reactive2(useScroll(element, __spreadProps$22(__spreadValues$72({}, options), {
...
function useIntersectionObserver(target, callback, options = {}) {
  ...
  let cleanup = noop;
  const stopWatch = isSupported ? watch2(() => ({
...
function useMediaControls(target, options = {}) {
  ...
  watchEffect2(() => {
    if (!document2)
      return;
    const el = unref2(target);
    if (!el)
      return;
    const src = unref2(options.src);
    let sources = [];
    if (!src)
      return;
    if (isString(src))
  ...
  watchEffect2(() => {
    if (!document2)
      return;
    const textTracks = unref2(options.tracks);
    const el = unref2(target);
  ...
  useEventListener(target, "timeupdate", () => ignoreCurrentTimeUpdates(() => currentTime.value = unref2(target).currentTime));
  useEventListener(target, "durationchange", () => duration.value = unref2(target).duration);
  useEventListener(target, "progress", () => buffered.value = timeRangeToArray(unref2(target).buffered));
  useEventListener(target, "seeking", () => seeking.value = true);
  useEventListener(target, "seeked", () => seeking.value = false);
  useEventListener(target, "waiting", () => waiting.value = true);
  useEventListener(target, "playing", () => waiting.value = false);
  useEventListener(target, "ratechange", () => rate.value = unref2(target).playbackRate);
...
function useOffsetPagination(options) {
  ...
  const pageCount = computed2(() => Math.ceil(unref2(total) / unref2(currentPageSize)));
...
function usePointer(options = {}) {
  ...
  return __spreadProps$12(__spreadValues$42({}, toRefs(state)), {
    isInside
  });
}
var SwipeDirection;
...
function useShare(shareOptions = {}, options = {}) {
  ...
  const share = async (overrideOptions = {}) => {
    if (isSupported) {
      const data = __spreadValues$32(__spreadValues$32({}, unref2(shareOptions)), unref2(overrideOptions));
      let granted = true;
...
function useSpeechRecognition(options = {}) {
  ...
  const lang = ref2(options.lang || "en-US");
...
function useTimeAgo(time, options = {}) {
  ...
  const timeAgo = computed2(() => getTimeago(new Date(unref2(time)), unref2(now2.value)));
  if (exposeControls) {
...
function useTitle(newTitle = null, options = {}) {
  ...
  const title = ref2((_a2 = newTitle != null ? newTitle : document2 == null ? void 0 : document2.title) != null ? _a2 : null);
  watch2(title, (t, o) => {
...
function useVModel(props, key, emit, options = {}) {
  ...
  if (passive) {
    ...
    watch2(() => props[key], (v) => proxy.value = v);
    watch2(proxy, (v) => {
      if (v !== props[key] || deep)
        _emit(event, v);
    }, {
